		...
		return (
			<Wrapper>
				...
				<MoneyTitle>
					...
					<MoneyValue>
						<span>{`${this.props.formatAmount(currentDeposite)} ${currency}`}</span>
					</MoneyValue>
				</MoneyTitle>
				<LastServiceWarpper>
					<Label><p>{e.payment_latest}</p><p>{e.payment_use}</p></Label>
					<Service href={forwardUrl} src={lastPaymentService ? `images/${lastPaymentService.src}` : 'images/pay-pal.png'} />
				</LastServiceWarpper>
				<ServicesWrapper>
					<div className='scrollable-wrapper'>
						{
							services.map((service, index) => {
								return (
									<Service href={forwardUrl} onClick={this.useService.bind(this, service)} settingId={service.settingId} key={index} src={`images/${service.src}`} />
								)
							})
						}
					</div>
