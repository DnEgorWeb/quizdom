...
export default class NicknameSet extends React.Component{
    ...
    submit = () => {
        if(this.state.phone){
            ...
            window.notification.alert(e.mydata_attention, e.mydata_phoneMustBeFilled, 'Ok', () => {})
        }
    }
    ...
    render(){
        return (
            <Wrapper>
                <TopBar caption={e.mydata_mobilePhoneNumber} />
                <TopPanel>
                    <TopText>
                        {e.formatString(
                            e.mydata_pleaseEnterPersonal,
                            <span className="email">{e.mydata_phone}</span>
                        )}
                    </TopText>
                </TopPanel>
                <MiddlePanel>
                    <Input placeholder={e.mydata_phone} value={this.state.phone} onChange={(e) => {this.inputHandler('phone', e.target.value)}} />
                    <SendButtonWrapper>
